...
import { getValidToken } from '../token-handler';
...
export const baseApi = async (endpoint: string, options: TRequestOptions = {}): Promise<any> => {
  const {
    method = 'GET',
    data,
  ...
  const url = new URL(`${baseApi}${endpoint}`);
  if (params) {
